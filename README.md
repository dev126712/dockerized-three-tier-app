...
|Job Name|Dependency|Purpose|Tool|
|build-image-frontend|secirity-scan-Checkov-frontend|uilds the Docker image for the Frontend and uploads it as an artifact.|Docker Buildx

build-image-backend

secirity-scan-Checkov-backend

Builds the Docker image for the Backend and uploads it as an artifact.
...
Builds the Docker image for the Proxy and uploads it as an artifact.
...
Builds the Docker image for the Database and uploads it as an artifact.
...
Downloads the Frontend artifact, loads it, and runs a Trivy vulnerability scan.
...
Downloads the Backend artifact, loads it, and runs a Trivy vulnerability scan.
...
Downloads the Proxy artifact, loads it, and runs a Trivy vulnerability scan.
...
Downloads the Database artifact, loads it, and runs a Trivy vulnerability scan.
...
Tags and pushes the secure Frontend image.

push-backend-image-to-dockerhub

scan-backend-with-trivy

Tags and pushes the secure Backend image.

push-proxy-image-to-dockerhub

scan-proxy-with-trivy

Tags and pushes the secure Proxy image.

push-database-image-to-dockerhub

scan-database-with-trivy

Tags and pushes the secure Database image.
...
Your Docker Hub username, used to prefix the repository names (e.g., username/app-backend).

DOCKER_HUB_TOKEN

A Docker Hub Access Token with Read, Write, and Delete permissions, used to log in and push the final images.
